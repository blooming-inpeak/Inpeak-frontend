...
export const ToggleSwitch = ({ isChecked, onClick }: ToggleProps) => {
  return (
    <ToggleWrapper $isChecked={isChecked} onClick={onClick}>
      <Slider $isChecked={isChecked} />
    </ToggleWrapper>
  );
};

const ToggleWrapper = styled.div<{ $isChecked: boolean }>`
  width: 36px;
  height: 22px;
